        ...
        <div class="uk-clearfix">
            <div class="uk-float-left">
                ...
                <div class="uk-margin-small-top">
                    <field-multipleselect bind="entry.{{ $fieldNames['nav'] }}" options="{ {main:'@lang('Main')',footer:'@lang('Footer')'} }"></field-multipleselect>
                </div>
            </div>

            <div class="uk-float-right">
                <div class="uk-margin-small-top">
                    <field-boolean bind="entry.{{ $fieldNames['published'] }}" label="@lang('Published')" class="label-left"></field-boolean>
                </div>

                @if($pageTypeDetection == 'type')
                <div class="uk-margin-top">
                    <label class="uk-text-middle uk-text-small">@lang('Page type')</label>
                    <select bind="entry.{{ $fieldNames['type'] }}" bind-event="input">
                        <option value=""></option>
...
{ (entry.{{ $fieldNames['subpagemodule'] }} = !entry.{{ $fieldNames['subpagemodule'] }} || Array.isArray(entry.{{ $fieldNames['subpagemodule'] }}) ? {} : entry.{{ $fieldNames['subpagemodule'] }}) && '' }
            <cp-fieldcontainer>
                ...
                <div class="{ entry.{{ $fieldNames['subpagemodule'] }}.active && 'uk-flex uk-flex-bottom' }">
                    <field-boolean bind="entry.{{ $fieldNames['subpagemodule'] }}.active" label="@lang('Sub pages')" class="uk-margin-small uk-text-middle small"></field-boolean>

                  @if($pageTypeDetection == 'collections')
                    <select bind="entry.{{ $fieldNames['subpagemodule'] }}.collection" aria-label="@lang('Select a collection for sub pages')" class="uk-margin-small uk-margin-small-left" title="@lang('Select collection')" if="{ entry.{{ $fieldNames['subpagemodule'] }}.active }">
                        <option value=""></option>
                  ...
                  @endif
                    ...
                    <a class="uk-icon-chevron-circle-{ mp_subpageToggle ? 'up' : 'down' } uk-icon-hover uk-text-large uk-margin-small-left" aria-label="@lang('Toggle dropdown')" onclick="{ mp_toggleSubpage }" if="{ entry.{{ $fieldNames['subpagemodule'] }}.active }"></a>
                </div>

                <div if="{ mp_subpageToggle && entry.{{ $fieldNames['subpagemodule'] }}.active }">
                @if($pageTypeDetection == 'collections' && count($collections))
                    <div class="uk-margin-small-top">
                        <div class="">
                            <label class="uk-text-small">@lang('Route')</label>
                            <input type="text" class="uk-form-width-small" bind="entry.{{ $fieldNames['subpagemodule'] }}.route{lang ? '_'+lang : ''}" />
                ...
                @elseif($pageTypeDetection == 'type')
                    <div class="uk-margin-small-top" if="{ entry.{{ $fieldNames['subpagemodule'] }}.active }">
                        <label class="uk-text-small">@lang('Type')</label>
                        <select bind="entry.{{ $fieldNames['subpagemodule'] }}.type" bind-event="input">
                            <option value=""></option>
                            @foreach($pageTypes as $type)
                            <option value="{{ $type }}">@lang(ucfirst($type))</option>
                            @endforeach
                ...
                @endif
                    ...
                    <div class="uk-margin-small-top">
                        <cp-fieldcontainer>
                            <div class="uk-panel">
                                <div class="uk-margin-small-top">
{ (entry.{{ $fieldNames['subpagemodule'] }}.display = typeof entry.{{ $fieldNames['subpagemodule'] }}.display == 'undefined' ? true : entry.{{ $fieldNames['subpagemodule'] }}.display) && '' }
                                    <field-boolean bind="entry.{{ $fieldNames['subpagemodule'] }}.display" label="@lang('Display block with sub pages')" class="small"></field-boolean>
                                </div>
                                <div class="uk-margin-small-top">
                                    <label class="uk-text-small">@lang('Limit')</label>
                                    <input type="number" bind="entry.{{ $fieldNames['subpagemodule'] }}.limit" placeholder="5" size="3" class="uk-form-width-mini" />
                                </div>
                                <div class="uk-margin-small-top">
                                    <field-boolean bind="entry.{{ $fieldNames['subpagemodule'] }}.sort" label="@lang('Reverse order')" class="small"></field-boolean>
                                ...
                                <div class="uk-margin-small-top">
                                    <field-boolean bind="entry.{{ $fieldNames['subpagemodule'] }}.pagination" label="@lang('Show pagination')" class="small"></field-boolean>
                                </div>
                                <div class="uk-margin-small-top">
                                    <label class="uk-text-small">@lang('Custom sort order')</label>
                                    <field-key-value-pair bind="entry.{{ $fieldNames['subpagemodule'] }}.customsort"></field-key-value-pair>
                                </div>
                            </div>
                        </cp-fieldcontainer>
                    </div>
        ...
        <div class="uk-panel uk-margin-small uk-width-medium-1-1 uk-width-small-1-2">
            <cp-fieldcontainer>
                <a href="#" aria-label="@lang('Toggle dropdown to set special page types')" class="uk-flex uk-flex-bottom uk-link-muted" onclick="{ mp_toggleSpecialPageTypes }">
                    ...
                    <i class="uk-icon-home uk-margin-small-left uk-text-small uk-text-muted" if="{ entry.{{ $fieldNames['startpage'] }} }"></i>
                    <i class="uk-icon-shield uk-margin-small-left uk-text-small uk-text-muted" if="{ entry.{{ $fieldNames['privacypage'] }} }"></i>
                    <span class="uk-flex-item-1"></span>
                ...
                <div class="uk-margin-small" if="{ mp_specialPageTypeToggle }">
                    <div class="uk-margin-small-top">
                        <field-boolean bind="entry.{{ $fieldNames['startpage'] }}" label="@lang('Home page')" class="small"></field-boolean>
                    </div>
                    <div class="uk-margin-small-top">
                        <field-boolean bind="entry.{{ $fieldNames['privacypage'] }}" label="@lang('Privacy page')" class="small"></field-boolean>
